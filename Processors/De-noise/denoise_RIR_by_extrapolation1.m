function y = denoise_RIR_by_extrapolation1(x, fs, doplot, hiF, loF)
...
% INPUT ARGUMENTS
%
% x contains a room impulse response. If it is a structure, then it
% requires x.audio (the room impulse response wave) and x.fs (the audio
...
if isstruct(x)
    ...
    % use 8 bands (125 Hz - 16 kHz) by default
    if nargin <4
        hiF = 16000;
        loF = 125;
    end
